...
double calculate_pid_output(PidController *pid, int target, int encoder, int max_pwm)
{
  ...
  // 计算PID输出 （位置式直接赋值）
  // pid->output =  error*50 +  pid->integral*20 + 20* error_rate;
  pid->output =
      error * pid->Kp + pid->integral * pid->Ki + pid->Kd * error_rate;

  // 保存上一次误差
  pid->last_error = error;
  // std::cout << "=================" << std::endl;
  // std::cout << "target: " << target << " encoder: " << encoder << std::endl;

  // std::cout << "error: " << error << " pid->integral: " << pid->integral <<
  // std::endl;

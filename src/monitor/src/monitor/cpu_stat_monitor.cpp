...
#include "monitor/CpuStat.h"

namespace monitor {
void CpuStatMonitor::UpdateOnce(monitor::MonitorInfo* monitor_info) {
    ...
    if (cpu_stat_list[0].find("cpu") != std::string::npos) {
      ...
      if (it != cpu_stat_map_.end()) {
        ...
        float cpu_soft_irq_percent = (cpu_stat.soft_irq - old.soft_irq) /
                                     (new_cpu_total_time - old_cpu_total_time) *
                                     100.00;
        cpu_stat_msg.cpu_name = cpu_stat.cpu_name;
        cpu_stat_msg.cpu_percent = cpu_percent;
        cpu_stat_msg.usr_percent = cpu_user_percent;
        cpu_stat_msg.system_percent = cpu_system_percent;
        cpu_stat_msg.nice_percent = cpu_nice_percent;
        cpu_stat_msg.idle_percent = cpu_idle_percent;
        cpu_stat_msg.io_wait_percent = cpu_io_wait_percent;
        cpu_stat_msg.irq_percent = cpu_irq_percent;
        cpu_stat_msg.soft_irq_percent = cpu_soft_irq_percent;
        monitor_info->cpu_stat.push_back(cpu_stat_msg);
      }

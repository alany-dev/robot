...
#include "monitor/NetInfo.h"

namespace monitor {
void NetMonitor::UpdateOnce(monitor::MonitorInfo* monitor_info) {
  ...
  while (net_file.ReadLine(&net_datas)) {
    ...
    if (name.find(':') == name.size() - 1 && net_datas.size() >= 10) {
      ...
      if (iter != net_info_.end()) {
        ...
        monitor::NetInfo one_net_msg;
        one_net_msg.name = net_info.name;
        one_net_msg.send_rate = (net_info.snd_bytes - old.snd_bytes) /
                                1024.0 / period;
        one_net_msg.rcv_rate = (net_info.rcv_bytes - old.rcv_bytes) /
                               1024.0 / period;
        one_net_msg.send_packets_rate =
            (net_info.snd_packets - old.snd_packets) / period;
        one_net_msg.rcv_packets_rate =
            (net_info.rcv_packets - old.rcv_packets) / period;
        monitor_info->net_info.push_back(one_net_msg);
      }

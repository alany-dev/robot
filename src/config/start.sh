...
if [ -z "$host_ip" ]; then
    ...
    # 在AP模式下启动WiFi配置Web服务
    echo "启动WiFi配置Web服务..."
    cd /home/orangepi/robot && nohup bash -c "source /opt/ros/noetic/setup.sh && source /home/orangepi/robot/devel/setup.sh && sleep 2  && python3 web_controller.py --ap-mode" > /tmp/web_controller.log 2>&1 &
    sleep 3  # 等待web服务启动
...
sleep 3
#启动all.launch
cd /home/orangepi/robot && nohup bash -c "source /opt/ros/noetic/setup.sh && source /home/orangepi/robot/devel/setup.sh && sleep 3  && python3 web_controller.py" > /tmp/web_controller.log 2>&1 &
sleep 1  # 等待web服务启动
...
cd /home/orangepi/robot
  taskset -c 1,2 nice -n 15   bash -iec "source /opt/ros/noetic/setup.sh && source /home/orangepi/robot/devel/setup.sh && sleep 3 && roslaunch base_control base_control.launch" > /tmp/base_control.log 2>&1 &

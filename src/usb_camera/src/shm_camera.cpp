...
int main(int argc, char** argv)
{
    ...
    shm_transport::Publisher shm_compressed_pub =
        shm_topic.advertise<sensor_msgs::CompressedImage>(compressed_topic, 1, 10 * 1024 * 1024);
    shm_transport::Publisher shm_raw_pub =
        shm_topic.advertise<sensor_msgs::Image>(raw_topic, 1, 20 * 1024 * 1024);
    ...
    while(nh.ok())
    {
        const int subscribers =
            (capture_format == CaptureFormat::kMjpeg)
                ? shm_compressed_pub.getNumSubscribers()
                : shm_raw_pub.getNumSubscribers();
        if (subscribers == 0 || enable_camera == 0)
        {
            ros::spinOnce();
            usleep(100 * 1000);
            continue;
        ...
        V4l2 v4l2;
        if (v4l2.init_video(dev_names[video_index].c_str(), width, height, capture_format, fps) <
            0)
        {
        ...
        while (nh.ok())
        {
            ...
            if (v4l2.active_format == CaptureFormat::kMjpeg)
            {
                ...
                shm_compressed_pub.publish(compressed_msg);
            }
            else
            {

...
for a list of pre-trained models to download.
  ...
  ros::NodeHandle nh;
  ros::Publisher llm_pub = nh.advertise<std_msgs::String>("/llm_request", 1);
  ...
  while (!stop && ros::ok()) {
    while (recognizer.IsReady(s.get()) && ros::ok()) {
      recognizer.DecodeStream(s.get());
    }

    if (!ros::ok() || stop) break;
    ...
    if (is_endpoint) {
      if (!text.empty()) {
        ...
        // Publish text to ROS topic
        std_msgs::String msg;
        msg.data = text;
        llm_pub.publish(msg);
        std::cout << "[voice -> llm] published: " << text << std::endl;
